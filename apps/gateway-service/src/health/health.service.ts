...
export class HealthService {
    ...
    async checkGatewayHealth(): Promise<HealthIndicatorResult> {
    ...
    async checkSecurityHealth(): Promise<HealthIndicatorResult> {
    ...
    async checkContainerHealth(): Promise<HealthIndicatorResult> {
        try {
    ...
    async checkAllHealth(): Promise<HealthCheckResult> {
        ...
        const status = results.every((result) => 
            Object.values(result)[0].status === 'up'
        );
