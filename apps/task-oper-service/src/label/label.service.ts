...
export class LabelService {
    ...
    async createLabel(callerId: string, projectId: string, createLabelObject: CreateLabelDto): Promise<LabelResponseDto>{
        try {
            
            const checkCallerBelongsToProject = await this.prismaService.project.findUnique({
                where: {
                    id: projectId,
                    workspace: {
                        workspaceMembers: {
            ...
            if(!checkCallerBelongsToProject){
                throw new ForbiddenException('You are not member of this project or workspace');
            }

            if(checkCallerBelongsToProject.workspace.workspaceMembers[0].role === 'VIEWER'){
            ...
            return new LabelResponseDto(labelCreated);
              ...
              if (error instanceof PrismaClientKnownRequestError && error.code === 'P2002') {
                    throw new ConflictException(`A label named '${createLabelObject.name}' already exists in this project`);
                }
    ...
    async getAllLabel(callerId: string, projectId: string): Promise<LabelResponseDto[]>{
        try {
            ...
            if(!callerUserExistence){
                throw new ForbiddenException('You do not have the permission to see labels');
            }
    ...
    async updateLabel(callerId: string, projectId: string, labelId: string, updateLabelObject: UpdateLabelDto): Promise<LabelResponseDto> {
        try {
            const checkCallerBelongsToProject = await this.prismaService.project.findUnique({
                where: {
                    id: projectId,
            ...
            if (!checkCallerBelongsToProject) {
                throw new ForbiddenException('You are not a member of this project or workspace');
            }

            if (checkCallerBelongsToProject.workspace.workspaceMembers[0].role === 'VIEWER') {
                throw new ForbiddenException('You do not have write access to update labels');
            }
            ...
            if (error instanceof HttpException) throw error;
            if (error instanceof PrismaClientKnownRequestError && error.code === 'P2002') {
                throw new ConflictException(`A label named '${updateLabelObject.name}' already exists in this project`);
            }
            if (error instanceof PrismaClientKnownRequestError) throw error;
            throw new InternalServerErrorException(
                error instanceof Error ? error.message : 'Internal Server Error'
    ...
    async deleteLabel(callerId: string, projectId: string, labelId: string): Promise<void> {
        try {
            const checkCallerBelongsToProject = await this.prismaService.project.findUnique({
                where: {
                    id: projectId,
            ...
            if (checkCallerBelongsToProject.workspace.workspaceMembers[0].role === 'VIEWER') {
                throw new ForbiddenException('You do not have write access to delete labels');
            }
