...
export class IssueService {
	...
	private async validateProjectAccess(callerId: string, projectId: string, writeAccessRequired: boolean = false) {
		...
		if (!callerProjectMember) {
			throw new ForbiddenException('You do not belong to this workspace or project');
		}
		...
		if (writeAccessRequired) {
			...
			if (userRole === 'VIEWER' && !isLead) {
				throw new ForbiddenException('Viewers do not have permission to modify issues');
			}
		}
	...
	private formatIssueDetailResponse(issue: any): IssueDetailResponseDto {
		return new IssueDetailResponseDto({
			...issue,
			labels: issue.issueLabels?.map((l: any) => l.label) || issue.labels?.map((l: any) => l.label) || [],
			subTasks: issue.subTasks?.map((st: any) => ({
				id: st.id,
				title: st.title,
	...
	private async findIssueWithRelations(issueId: string, projectId: string): Promise<IssueDetailResponseDto> {
	...
	async createIssue(callerId: string, projectId: string, dto: CreateIssueDto): Promise<IssueDetailResponseDto> {
		try {
			const projectMember = await this.validateProjectAccess(callerId, projectId, true);
			...
			if (!status) {
				throw new NotFoundException('The status id does not exists on the project');
			}

			if (dto.assigneeId) {
				const existsAssignee = await this.prismaService.workspaceMember.findFirst({
					where: {
				...
				if (!existsAssignee) {
					throw new NotFoundException('The assignee does not belong to this workspace or project');
				}
			}
			...
			if (dto.epicId) {
				...
				if (!existsEpic) {
					throw new NotFoundException('The epic doesn not found on the project');
				}
			}
			...
			if (dto.sprintId) {
				...
				if (!existsSprint) {
					throw new NotFoundException('The Sprint does not exists on the project');
				}
			}
			...
			if (dto.labelIds && dto.labelIds.length > 0) {
				...
				if (labelCount !== dto.labelIds.length) {
					throw new NotFoundException('One or more labels do not exist in this project');
				}
			}

			const createdIssueId = await this.prismaService.$transaction(async (tx) => {
				...
				await this.activityService.logActivity(tx, issue.id, callerId, ActivityType.ISSUE_CREATED);
	...
	async getIssues(callerId: string, projectId: string, filters?: { sprintId?: string, epicId?: string, assigneeId?: string, statusId?: string, priority?: IssuePriority }): Promise<IssueListResponseDto> {
		try {
	...
	async getIssue(callerId: string, projectId: string, issueId: string): Promise<IssueDetailResponseDto> {
		try {
			await this.validateProjectAccess(callerId, projectId, false);
	...
	async updateIssue(callerId: string, projectId: string, issueId: string, dto: UpdateIssueDto): Promise<IssueDetailResponseDto> {
		try {
			const projectMember = await this.validateProjectAccess(callerId, projectId, true);
			...
			if (dto.statusId && dto.statusId !== oldIssue.statusId) {
				const status = await this.prismaService.status.findUnique({ where: { id: dto.statusId, projectId } });
			...
			if (dto.assigneeId && dto.assigneeId !== oldIssue.assigneeId) {
				const member = await this.prismaService.workspaceMember.findFirst({
					where: { userId: dto.assigneeId, workspaceId: projectMember.workspaceId }
				});
				if (!member) throw new NotFoundException('Assignee is not a workspace member');
			}

			if (dto.epicId && dto.epicId !== oldIssue.epicId) {
				const epic = await this.prismaService.epic.findUnique({ where: { id: dto.epicId, projectId } });
				if (!epic) throw new NotFoundException('Epic not found');
			}

			if (dto.sprintId && dto.sprintId !== oldIssue.sprintId) {
				const sprint = await this.prismaService.sprint.findUnique({ where: { id: dto.sprintId, projectId } });
				if (!sprint) throw new NotFoundException('Sprint not found');
			}

			if (dto.parentId && dto.parentId !== oldIssue.parentId) {
				const parent = await this.prismaService.issue.findUnique({ where: { id: dto.parentId, projectId } });
				if (!parent) throw new NotFoundException('Parent issue not found');
				if (parent.parentId !== null) throw new BadRequestException('Cannot nest subtasks more than one level deep');
			}

			const changes: { type: ActivityType, oldValue?: string, newValue?: string }[] = [];

			if (dto.statusId && dto.statusId !== oldIssue.statusId) {
				const newStatus = await this.prismaService.status.findUnique({ where: { id: dto.statusId } });
				changes.push({ type: ActivityType.STATUS_CHANGED, oldValue: oldIssue.status?.name, newValue: newStatus?.name });
			}
			if ('assigneeId' in dto && dto.assigneeId !== oldIssue.assigneeId) {
				const newAssignee = dto.assigneeId ? await this.prismaService.user.findUnique({ where: { id: dto.assigneeId } }) : null;
				const oldName = oldIssue.assignee ? `${oldIssue.assignee.firstName} ${oldIssue.assignee.lastName}` : 'Unassigned';
				const newName = newAssignee ? `${newAssignee.firstName} ${newAssignee.lastName}` : 'Unassigned';
				changes.push({ type: ActivityType.ASSIGNEE_CHANGED, oldValue: oldName, newValue: newName });
			}
			if (dto.priority && dto.priority !== oldIssue.priority) {
				changes.push({ type: ActivityType.PRIORITY_CHANGED, oldValue: oldIssue.priority, newValue: dto.priority });
			}
			if ('epicId' in dto && dto.epicId !== oldIssue.epicId) {
				const newEpic = dto.epicId ? await this.prismaService.epic.findUnique({ where: { id: dto.epicId } }) : null;
				changes.push({ type: ActivityType.EPIC_CHANGED, oldValue: oldIssue.epic?.title || 'None', newValue: newEpic?.title || 'None' });
			}
			if ('sprintId' in dto && dto.sprintId !== oldIssue.sprintId) {
				const newSprint = dto.sprintId ? await this.prismaService.sprint.findUnique({ where: { id: dto.sprintId } }) : null;
				changes.push({ type: ActivityType.SPRINT_CHANGED, oldValue: oldIssue.sprint?.name || 'Backlog', newValue: newSprint?.name || 'Backlog' });
			}
			if (dto.title && dto.title !== oldIssue.title) {
				changes.push({ type: ActivityType.TITLE_CHANGED, oldValue: oldIssue.title, newValue: dto.title });
			}
			if ('dueDate' in dto && dto.dueDate !== oldIssue.dueDate) {
				changes.push({ type: ActivityType.DUE_DATE_CHANGED, oldValue: oldIssue.dueDate?.toISOString(), newValue: dto.dueDate?.toISOString() });
			}
			...
			await this.prismaService.$transaction(async (tx) => {
				...
				for (const change of changes) {
					await this.activityService.logActivity(tx, issueId, callerId, change.type, change.oldValue, change.newValue);
				}
			});
	...
	async deleteIssue(callerId: string, projectId: string, issueId: string): Promise<void> {
		try {
	...
	async reorderIssue(callerId: string, projectId: string, issueId: string, dto: ReorderIssueDto): Promise<IssueDetailResponseDto> {
		try {
			await this.validateProjectAccess(callerId, projectId, true);
			...
			await this.prismaService.$transaction(async (tx) => {
				...
				if (dto.statusId !== existingIssue.statusId) {
					await this.activityService.logActivity(tx, issueId, callerId, ActivityType.STATUS_CHANGED, existingIssue.status.name, newStatusName);
				}
			});
	...
	async moveToSprint(callerId: string, projectId: string, issueId: string, dto: MoveSprintDto): Promise<IssueDetailResponseDto> {
		try {
			await this.validateProjectAccess(callerId, projectId, true);
			...
			if (dto.sprintId !== null) {
				...
				if (sprint.status === 'COMPLETED') {
					throw new BadRequestException('Cannot move issue to a completed sprint');
				}
	...
	async addLabel(callerId: string, projectId: string, issueId: string, labelId: string): Promise<IssueDetailResponseDto> {
		try {
			await this.validateProjectAccess(callerId, projectId, true);

			const issue = await this.prismaService.issue.findUnique({ where: { id: issueId, projectId } });
			if (!issue) throw new NotFoundException('Issue not found');

			const label = await this.prismaService.label.findUnique({ where: { id: labelId, projectId } });
	...
	async removeLabel(callerId: string, projectId: string, issueId: string, labelId: string): Promise<IssueDetailResponseDto> {
		try {
			await this.validateProjectAccess(callerId, projectId, true);
	...
	async getActivities(callerId: string, projectId: string, issueId: string): Promise<IssueActivityListResponseDto> {
		try {
			await this.validateProjectAccess(callerId, projectId, false);
