  ...
  private readonly context: string = SprintService.name;
	...
	private async validateProjectAccess(callerId: string, projectId: string, writeAccessRequired: boolean = false) {
		...
		if (writeAccessRequired) {
			...
			if (!isLead && !isOwner) {
				throw new ForbiddenException('Only Project Lead or Owner can perform this action');
			}
		}
	...
	async createSprint(callerId: string, projectId: string, createSprintObject: CreateSprintDto): Promise<SprintResponseDto> {
		try {
			...
			if(!createdSprint){
				throw new InternalServerErrorException('Due to server failure the sprint could not be created');
			}
	...
	async getAllSprints(callerId: string, projectId: string): Promise<SprintListResponseDto> {
		try {
	...
	async getSprint(callerId: string, projectId: string, sprintId: string): Promise<SprintResponseDto> {
		try {
			await this.validateProjectAccess(callerId, projectId, false);
	...
	async updateSprint(callerId: string, projectId: string, sprintId: string, updateSprintObject: UpdateSprintDto): Promise<SprintResponseDto> {
		try {
			await this.validateProjectAccess(callerId, projectId, true);
	...
	async startSprint(callerId: string, projectId: string, sprintId: string, startSprintObject: StartSprintDto): Promise<SprintResponseDto> {
		try {
			await this.validateProjectAccess(callerId, projectId, true);
			...
			if (activeSprint) {
				throw new ConflictException('Another sprint is already active in this project. You must complete it before starting a new one.');
			}
	...
	async completeSprint(callerId: string, projectId: string, sprintId: string, completeSprintObject: CompleteSprintDto): Promise<SprintResponseDto> {
		try {
			await this.validateProjectAccess(callerId, projectId, true);
			...
			if (completeSprintObject.destinationSprintId) {
				...
				if (!destSprint || destSprint.status !== 'PLANNED') {
					throw new ConflictException('Destination sprint must exist and be in PLANNED status');
				}
			}
	...
	async deleteSprint(callerId: string, projectId: string, sprintId: string): Promise<void> {
		try {
			await this.validateProjectAccess(callerId, projectId, true);
	...
	async getSprintIssues(callerId: string, projectId: string, sprintId: string): Promise<any> {
		try {
			await this.validateProjectAccess(callerId, projectId, false);
