 ...
 */
  ...
  async refreshMyToken(validatePayload: RefreshTokenResponse): Promise<CreateTokenDto>{
    //Making call to token service to generate new access and refresh token.
    const result = await this.tokenService.refreshToken(validatePayload.refreshToken,validatePayload);
    if(!result){
