...
import { RefreshTokenDto } from "src/token/dto/create-refresh-token.dto";
...
export class JwtExceptionFilter implements ExceptionFilter {
    ...
    async catch(exception: TokenExpiredError | JsonWebTokenError, host: ArgumentsHost) {
        ...
        if(request.url.includes('/users/refresh')){
            ...
            const expiredToken:RefreshTokenDto = request.cookies?.refreshToken || request.body?.refreshToken;
            if(expiredToken){
                await this.tokenService.invalidateRefreshToken(expiredToken);
        ...
        console.log('--------------------Error at JWT Exception--------------------');
        console.log(`${exception.name}: ${exception.message}`);
        console.log(message);
        console.log('--------------------Error at JWT Exception--------------------');
        console.error(exception);
        console.log('----------------END OF Error at JWT Exception-----------------');

        response.status(status).json(new CustomErrorResponseDto(status,request.url,message,errorCode));
    }
}