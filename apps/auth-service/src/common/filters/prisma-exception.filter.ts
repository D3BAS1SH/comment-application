...
export class PrismaClientExceptionFilter extends BaseExceptionFilter {
        ...
        switch(exception.code){
            ...
            case 'P2025': // An operation failed because a required record was not found
                status = HttpStatus.NOT_FOUND;
                errorMessage = 'The requested resource or a related one could not be found.';
                break;
            ...
            case 'P2034': // Transaction conflict: a write conflicts with a previous write.
                status = HttpStatus.CONFLICT;
                errorMessage = 'The operation could not be completed due to a data conflict. Please try again.';
                break;
            
            // --- 503 Service Unavailable: Transaction API errors ---
            case 'P2028': // Transaction API error (e.g., transaction timed out)
                status = HttpStatus.SERVICE_UNAVAILABLE;
                errorMessage = 'The database service is temporarily unavailable or timed out. Please try again later.';
                break;

            default: {
                console.log('--------------------Error at Prisma Exception--------------------');
                console.log(`${exception.code}: ${exception.message}`);
                console.log(message);
                console.log('--------------------Error at Prisma Exception--------------------');
                console.error(exception);
                console.log('----------------END OF Error at Prisma Exception-----------------');
                break;
            }
        }
